...
- 解析 Chat 中能随 Provider 消息传入的文件引用、文件内选中范围和粘贴文本：选中内容会以有界文本上下文传给 Qoder，完整文件会保留路径/URI并在需要时通过 `read_file` 延迟读取，避免大文件一次性撑爆上下文；当 VS Code 只在 UI 中保留引用芯片时，自动补充当前活动编辑器的文件和选区。
...
- 当前始终使用 Language Model Provider：选择 Qoder 模型后直接发送消息，不需要 `@qoder` 或其他手动入口。VS Code Provider 回调拿不到独立的 `ChatRequest.references`；如果引用芯片没有进入消息内容，扩展只能用当前活动编辑器文件/选区兜底，无法猜测未打开的任意外部文件。
